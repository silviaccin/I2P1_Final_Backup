...
static void fail_draw(Scene* sc) {
    FailSceneData* data = (FailSceneData*)(sc->pDerivedObj);
...
static void fail_update(Scene* sc) {
    FailSceneData* data = (FailSceneData*)(sc->pDerivedObj);
    ALLEGRO_EVENT ev;
...
static void fail_destroy(Scene* sc) {
    FailSceneData* data = (FailSceneData*)(sc->pDerivedObj);
...
Scene* New_FailScene() {
    ...
    FailSceneData* data = (FailSceneData*)malloc(sizeof(FailSceneData));
    GAME_ASSERT(data);
    data->selected = 0;

    data->image = al_load_bitmap("assets/images/fail.png");
    if (!data->image) fprintf(stderr, "[WARN] Failed to load fail.png\n");

    data->sound = al_load_sample("assets/sound/fail.wav");
    if (data->sound) al_play_sample(data->sound, 1.0, 0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
    else fprintf(stderr, "[WARN] Failed to load fail.wav\n");
    ...
    sc->scene_end = false;
    sc->pDerivedObj = data;
    sc->Update = fail_update;
    sc->Draw = fail_draw;
    sc->Destroy = fail_destroy;
